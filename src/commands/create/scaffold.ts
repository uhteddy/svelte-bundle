...
import type { OptionalFeature, PackageManager, ScaffoldContext } from '../../types.ts';
...
export async function scaffold(ctx: ScaffoldContext): Promise<void> {
  ...
  await writeFile(join(ctx.targetDir, 'package.json'), buildPackageJson(ctx), 'utf-8');
  await writeFile(join(ctx.targetDir, 'vite.config.ts'), buildViteConfig(ctx), 'utf-8');
  ...
  if (ctx.git) {
    ...
    if (gitResult.status === 0) {
      ...
      spawnSync('git', ['commit', '-m', 'chore: initial commit from svelte-bundle'], {
        cwd: ctx.targetDir,
        stdio: 'pipe',
      });
  ...
  if (ctx.install) {
    ...
    if (installResult.status !== 0) {
      logger.warn('Dependency installation failed. Run the install command manually.');
    } else {
